 ...
 *  its own; it cannot tell the account whose install crew is booked
...
import { QUEUES, type ActionRow } from "@/data/action-center";
...
export interface CustomerNotice {
   ...
   *
   * One PO carries one style to one account order, so this is a name rather than
   * a count — "Peachtree Jewelers has not been told" says who to go and
...
export function noticeFor(row: ActionRow): CustomerNotice | null {
  /* A account order and the purchase order behind it now share a number —
     SO-4463 is the outbound half of PO-4463 — so the pairing is the reference
     itself. The style fallback stays for orders that name the same product but
 ...
 * share a style with a account order without changing anything the account was
 ...
 * A account order and the purchase order behind it are one event seen
 * from two seats. SO-4471 is 120 units of Floorté Pro SPC ·
 * Diffuse Color for Peachtree; PO-4471 is the same 120 units from Binh
 * Duong. When the plant moves, both seats are looking at that move —
 * and they should be saying the same thing about it.
 * ─────────────────────────────────────────────────────────────── */

/**
 * The buying row a service row is waiting on.
 *
 * Read from the row's own `chainFrom` rather than matched on style. Style
 * matching held only while every seat spelled a product identically, and it did
 * not: the planner's line for this same SKU was filed under "Soft surface ·
 * Commercial" where the buyer had "Bradshaw Chronograph 43", so the link
 * silently went missing on exactly the row it mattered most for. A named
 * reference cannot drift.
 *
 * The style match stays as a fallback for rows that predate the explicit link —
 ...
 * A purchase order feeds a account order; a claim is filed against
 ...
 * purchase order ends at a Fossil DC, so a load carrying one is inbound; a sales
 * order leaves a DC for a account, so a load carrying one is outbound. On a
 * logistics queue that is not decoration — it is the difference between a
 * drayage off a vessel and a delivery a account is waiting at home for, and the
 * two are worked by different people against different clocks.
 *
...
export interface LinkedOrder {
  ref: string;
  /** True for a purchase order — goods coming into a Fossil DC. */
  inbound: boolean;
}
...
export function linkedOrderOf(row: ActionRow): LinkedOrder | null {
  ...
  /* A account order looks the other way: what a CSR wants beside SO-4471 is the
