 ...
 *  Neither shows the WORKING, and the working is what a planner
 *  approving $112K of stock is entitled to: which factors fed the
...
export function waterfallFor(e: Exception): DeckStep[] {
  ...
  return [
    { label: "On hand", value: `${e.onHand} ctn`, sub: `At ${e.branch} today` },
    {
      label: "Target stock",
      delta: "−",
      value: `${target} ctn`,
      sub: `${lead} of lead-time demand plus a ${e.safetyStock}-carton buffer`,
    },
    {
      label: "Excess above target",
      value: `${excessOf(e)} ctn`,
...
export function deckFor(e: Exception): DemandDeck {
  ...
  return {
    exception: e,
    summary: short
      ? `${e.sku} at ${e.branch} holds ${e.onHand} units against a ${target}-carton target — ${cover} days of cover at the current POS run rate, on a ${e.leadTimeDays}-day lead from ${e.vendor}. ${e.incoming ? `${e.incoming} units are already travelling and close part of the gap.` : "Nothing is on order."} I recommend requesting ${e.requestedQty} to buying.`
      : `${e.sku} at ${e.branch} holds ${e.onHand} units against a ${target}-carton target — ${excessOf(e)} units more than policy asks for, with ${e.incoming} still inbound. These exist and are in the wrong place, so I recommend moving ${e.requestedQty} to ${e.transferTo ?? "the other centre"} rather than buying any more.`,
