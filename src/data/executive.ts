 ...
 *
 * Everything here reads across every position at both DCs and across all
 * twelve supplying sites, because that is what makes these figures this seat's
 * rather than a desk's: a planner knows one DC, a buyer knows one category,
...
import { SUPPLIERS, money, type Supplier } from "@/data/buying";
import {
  ACCESSORIALS,
  LANES,
  detentionNow,
...
import { QUEUES, linesFor, parseCalendar } from "@/data/action-center";
...
/**
 * Owned-brand gross margin, for turning cost into revenue.
 *
 ...
 * 34% is an owned-brand blend — the margin the owned-brand model exists to
 * capture, and the reason this portfolio is designed in-house and bought FOB
 * rather than filled from national brands.
 *
...
/**
 * The portfolio's annual revenue, across every category on the sheet.
 *
 * A real anchor, and the reason it is here: the derived book is the home &
 * kitchen book only — 396 positions grossing up to about $261M a year — and an
 * executive dashboard showing that as "revenue" reads as the whole portfolio.
 * The owned-and-partner-brand book this seat answers for turns over about
 * $1.2B a year across home, grocery and softlines. Off by a factor of nearly
 * five is not a rounding.
 *
 * So this anchors the page at portfolio scale and `CATEGORY_MULTIPLE` carries the
 ...
 */
/** Sized as a category P&L, not the retailer: a mass merchant turns over two
 *  orders of magnitude more than any one merchandising portfolio, and claiming
 *  the storewide figure here would be a sourcing this demo does not carry.
 *  Deliberately round for the same reason. */
...
const TARGETS = {
  /* Annualised revenue plan, at the margin above. Set against the three-RDC
     network — it was 135M when the book was two DCs, and left revenue
     reading +94% against plan, which is not a variance, it is a stale plan. */
  revenue: FOSSIL_ANNUAL_REVENUE * 1.03,
  /** Landed-cost budget for the period — duty in, 301 included. */
  cost: 820_000,
  /* Scaled with everything else — see categoryMultiple. */
  forecastAccuracy: 85,
  /* A plan a general-merchandise book would actually set. The old 10 was
     ...
     thing this seat is trying to do (the dated grocery lines turn far faster
     than five; the imported home book is what holds the blend down), so the
     tile should be able to show it working. */
  inventoryTurns: 5,
} as const;
 ...
 */
/** The home & kitchen book's own revenue — derived, and the base the multiple is off. */
export function carpetTileRevenue(): number {
  return Math.round((annualCogs() * YTD_FRACTION) / (1 - GROSS_MARGIN));
}

/**
 * How many times the loaded book fits into the portfolio's whole one.
 *
 ...
 * prototype loads the priority book and cannot know how the rest divides.
 */
/**
 * What each category is of the portfolio's revenue.
 *
 * Approximate, from the merchandising plan — home & kitchen runs just under
 * four-fifths of this portfolio, the grocery co-pack book around an eighth,
 * softlines the rest. Deliberately round: precise decimals would claim
 * a sourcing this demo does not carry.
 *
 * Zero where zero is the fact: fixtures & packaging is spend, not revenue,
 * and pharmacy is exited — run by a pharmacy partner in-store since 2015 —
 * so an executive reading either at a revenue scale should see the zero and
 * know why.
 */
 ...
 *
 * All categories is the whole portfolio — the loaded book times
 * `categoryMultiple`. A named category is the portfolio times that category's
 * revenue share: Home & Kitchen reads ~$950M a year of a ~$1.2B portfolio,
 * because that is roughly what the owned-brand home book does here, and a
 * control that printed the fixture book's own $64M under the label
 * "Home & Kitchen" would be a scale error wearing a scope's clothes. (An
 * earlier cut did exactly that — the ×1 "loaded book" reading is honest about
 * the fixtures and wrong about the world, and an executive page answers for
 * the world.)
 *
 * This is the executive's ONE control. Brand left the model's controls when
 * the West Priority sheet turned out not to plan by it; category is the
...
export function ytdRevenue(scale = categoryMultiple()): number {
  /* Revenue is the home & kitchen book grossed up, times whatever scale is in
     force — so All categories lands on the portfolio's own figure and Home &
     Kitchen lands on the book that was actually derived. */
 ...
 * answer with them too, and a chip reading "$1.24B" beside a tile reading
 * "$1236.0M" is the same fact told two ways.
 *
 ...
 * tiles use — so a chip cannot answer "$1236.0M" beside a tile reading
 * "$1.24B", and neither can drift when the fixtures move.
 *
  ...
  return [
    {
      ...
      basis:
        scale === 1
          ? "The home & kitchen book, at cost, grossed up"
          : `All categories · home & kitchen is ${money(carpetTileRevenue())} of it`,
      targetDerived: false,
    },
    {
      id: "cost",
    ...
    },
    {
      ...
      better: "higher",
      basis: "Stock on hand at both DCs, at cost",
      targetDerived: true,
    },
    {
      id: "forecastAccuracy",
 ...
 * figure on the page and the only one tied to individual receipts — and on the
 * China-origin lines it is where the Section 301 exposure actually lands, duty
 * paid at the port rather than duty modeled on a sheet.
 */
export function landedCost(): number {
  return receivedLines().reduce((sum, l) => sum + l.value, 0);
}

/** Units behind that cost, so the figure carries its own base — the period's
 *  receipt throughput at the DCs. */
...
/**
 * Under-stock and over-stock, by the DC that holds it.
 *
 ...
 * compares side by side — they are two lists with the same shape, and a DC
 * usually appears on both.
 *
 * Counting positions in trouble, not positions held: "7 products" means seven
 * decisions waiting at that DC. And the value is those units at cost in
 * both directions, but it means different things — short is revenue that cannot
 * be served, long is capital that cannot be spent, and on the dated grocery
 * book long stock has a write-off date printed on the bag — so the column is
 * renamed by the toggle rather than left as a neutral "value" that flatters
 * both.
 *
 ...
 * retail book does. Ranking a flat distribution produces a leaderboard with no
 * leader. Grouping by DC and by direction asks a question the data can
 * actually answer.
 */
  ...
  return branches
    .map((branch) => {
      ...
      /* ALL THREE columns scale together — see `categoryMultiple`.
         An earlier build scaled the dollars and left the counts "as measured",
         ...
         premise is that the loaded book stands in for a slice of the
         portfolio; the counts make that claim or the dollars cannot. At the
         loaded book's own scale (×1) every figure is the derived book's own,
         unscaled. */
      return {
        branch,
 ...
 * called "Luen Hing Housewares" and two of those were in Vietnam, because a
 * commit that renamed the dedicated sites for their towns applied the name to
 * the import suppliers as well. This function keyed on `site` to route around
 * that, and the workaround outlived the bug — the names are distinct company
 * names again, and a supplier table should say who the supplier is.
 *
 * The dedicated co-pack sites are the exception, and deliberately: a dedicated
 * line is referred to by its town, so the site carries it with its state —
 * "River Falls, WI" rather than "River Falls" — which is how a buyer refers to
 * a plant. The country flag beside it does the rest of the geography.
 */
...
export function freightMonthly(scale = categoryMultiple()): {
  ...
  /* At portfolio scale, like the rest of the executive book. The lane fixture
     is eight lanes against the loaded book; the freight bill for the rest of
     the portfolio is the same ratio, and leaving it unscaled would have put a
     $100K freight line under a $736M revenue line on the same card. */
  const m = scale;
  return {
...
/**
 * How long an account waits, ordered to delivered.
 *
 * The one measure on this card that is the customer's experience rather than
 * the retailer's cost, which is why it is here: a freight bill coming down while
 * this number goes up is not efficiency, it is the promise being spent.
 *
