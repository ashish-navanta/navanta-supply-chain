/* ═══════════════════════════════════════════════════════════════
 *  Planning — the IRIS model, on Target's book
 *
...
import { DC_NAMES, SKUS } from "@/data/catalogue";
...
export type { Classification } from "@/data/action-center";
...
export const ABC_ROW_META: Record<Abc, { label: string; sub: string }> = {
  A: { label: "A", sub: "Top 80% Revenue" },
  B: { label: "B", sub: "Next 15% Revenue" },
  C: { label: "C", sub: "Bottom 5% Revenue" },
};
...
/**
 * What one unit of a given SKU costs at wholesale, wherever it is standing.
 *
 * Hashed on the SKU alone. It used to be hashed on sku@branch, which made the
 * same dinnerware set cost $7 at Woodland and $26 at Cedar Falls — invisible
 * while nothing printed a unit price, and nonsense the moment the overstock
 * table started valuing excess.
 *
 * $2–30, which is where this book's FOB costs actually sit — a 16-piece
 * stoneware set lands near $21.50, a granola pouch under $2 — and the retail
 * multiple puts them at $3.99–$99.99 on the shelf.
 */
export function cartonCost(sku: string): number {
  return 2 + Math.round(hash01(`cost|${sku}`) * 28);
}

/* The three regional distribution centres, from the catalogue rather than a
   list of their own. The fixtures used to name twelve invented sites —
   "Nashville DC-21", "Bakersfield DC-40" — which is not a scope anybody can
   hold in their head and was inventing geography besides. See DCS. */
const BRANCHES = DC_NAMES;

/** How many distribution centres the book is spread across. */
...
function generated(): SkuPolicyRow[] {
  ...
  /* Per COLOURWAY, not per style. A position is a SKU at a centre — that is
     what the key has always said and what the planner orders against — and
     generating one row per style meant eight distinct numbers standing in for
     thirty-five. It also kept the book populated when the twelve invented
     branches folded down to three real centres: eight styles across three
     centres is twenty-four positions, which is not a book. */
      ...
      out.push({
        ...
        ...(flipped
          ? {
              ...
              reason: "Held to demand while the new line beds in",
            }
          : {}),
        onHand: 0,
 ...
 * position goes wrong: an X line is smooth and forecastable, a Z line is lumpy
 * and intermittent and no amount of attention makes an order nobody could
 * predict arrive on time. ABC decides what it COSTS when it does, because an A
 * line carries the revenue.
 *
...
export const POSITIONS: SkuPolicyRow[] = (() => {
  ...
  /* A seeded row wins its sku@branch outright. The grid generates a position for
     every style at every branch, so wherever the planner's queue already holds
     one — SKU HH5605-5605 at Woodland RDC, say — the grid made a second with the
     same SKU and the same branch. Two rows for one position: a product page listed
     the branch twice with different figures, and React saw two children fighting
     over one key. The queue's own row is the real one; the generated twin goes. */
  const claimed = new Set(seeds.map((r) => `${r.sku}@${r.branch}`));
  return [...seeds, ...rest.filter((r) => !claimed.has(`${r.sku}@${r.branch}`))];
})();

/**
 * The other distribution centre.
 *
 * Target runs three RDCs in this prototype, so "somewhere else" always has an
 * answer and a long position always has a destination — the first centre that
 * is not this one. Written as a lookup rather than an index flip so a fourth
 * centre is a data change, not a logic change.
 */
...
/**
 * Working capital tied up in stock: every carton on hand, at cost.
 *
 * The one figure a supply-chain VP is held to that nobody below them owns — each
 * planner sees their own centre, and the total only exists across all of them.
 */
...
/**
 * How much of Target's inventory these DC positions actually represent.
 *
 * About a third, and the missing two-thirds are not a modelling gap — they are
 * the parts of the balance sheet this app does not hold positions for:
 *
 *   · IN-TRANSIT. The import book sails FOB Asia, and an ocean leg is weeks
 *     long, so a large block of owned stock is on the water at any moment.
 *   · STORES. Nearly two thousand doors carry their own shelf and backroom
 *     stock, none of it a DC position.
 *   · THE REST OF THE NETWORK. The full estate is ~50 facilities — food DCs
 *     and flow centers among them — and the prototype loads one node per
 *     region rather than pretending to load them all.
 ...
 * EXCEPTION book — deliberately lean, since the demo is about SKUs short of
 * cover. Dividing a year of company COGS by only the thin part of the
 * inventory returned 17× — stock turning every three weeks, which no
 * general-merchandise retailer does and which read as a triumph (+71% vs plan)
 * rather than as the arithmetic error it was.
 */
const DC_SHARE_OF_INVENTORY = 0.35;

/**
 * Inventory turns: a year of demand at cost, over the stock held to serve it.
 *
 * The textbook ratio — cost of goods sold ÷ average inventory — taken against
 * total inventory rather than against the DC slice alone. `annualValue` is
 ...
 *
 * Lands near 6× for the loaded book, which is where a big-box general
 * merchandiser genuinely sits — grocery pulls the blend up, home and seasonal
 * pull it down — so this is the plausible middle rather than a figure read
 * off a filing.
 */
...
/** Cartons above target — what should not be here. */
...
export type Exception = SkuPolicyRow & {
   ...
   * the wrong place, and the answer is the other centre. Carried on the row so
  ...
  .map((p) => {
    ...
    return {
      ...
      /* Two different moves, and they leave by different doors.
         SHORT is a requisition: deployment decides what a centre needs and
         buying places it, so "Request 193 from Luen Hing Housewares" rather than
         "Raise 193 on" — a planner cannot commit Target to a purchase.
         LONG is a transfer. The units already exist and are simply in the
         wrong place, so the move is to the other centre, not out of the network.
         "Move 9 out" said neither where to nor why, and read like a write-off. */
    ...
    for (const xyz of XYZ_ORDER) {
      ...
      cells.push({
        ...
        /* By concentration, not by presence.
           "The worst thing in the cell" turned all nine boxes red: a dozen-odd
           critical positions scattered across a hundred meant every cell held at
           least one, and a nine-box where every box is an exception cannot do the
           one thing it exists for — show WHERE the exposure sits. Two-thirds of
           the book is healthy and the matrix was saying none of it was.
...
export function planningRollup(positions = POSITIONS) {
  ...
  return {
    ...
    /* Distinct colourways in whatever slice was passed, not the style count. A
       position is a SKU at a centre now, and a page scoped to one centre should
       say how many products IT holds. */
...
export function traceFor(r: SkuPolicyRow): TraceStep[] {
  ...
  return [
    ...
    },
    {
      ...
      formula: "ABC by revenue tier · XYZ from CV²",
      inputs: [
        { label: "Revenue", value: `$${Math.round(r.annualValue / 1000)}K` },
