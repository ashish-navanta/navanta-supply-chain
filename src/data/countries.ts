 ...
 *
 *  The fixtures name countries several different ways — "US" and
 *  "United States", "CN" and "China", "Vietnam", "HK" — which is
 *  three notations in one column and no way to sort or group it. One
 *  record per country, keyed by both the code and the name, so
 *  whichever form a fixture holds resolves to the same row.
 *
 *  These are SOURCING ORIGINS, and origins stay countries even
 *  though the selling scope above them is cut by US state: goods are
 *  made in China, Vietnam or the US whatever state they sell in.
 *
 ...
 *
 * Regions — "West", "East" — are not countries and resolve to nothing, which
 * is the honest answer: the caller falls back to printing the region as text
 * rather than flying a flag for a third of the network.
 */
