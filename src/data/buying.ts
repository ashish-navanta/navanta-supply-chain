 ...
 *  dedicated co-manufacturing plant at River Falls, three legacy
 *  sourcing vehicles still on the corporate entity list, raw
...
export interface Supplier {
   ...
   *  differently. Four of these exist, and three are the entity-list
   *  question. */
  own: boolean;
  site: string;
  country: string;
  region: "South China" | "Vietnam" | "US Midwest" | "Other";
...
export const SUPPLIERS: Supplier[] = [
  ...
  S({
    ...
    scoreLines: LINES(76, 68, 79, 78, 62, [
      ...
      "Second weaving line free — capacity to take the peak-season surge",
  ...
  /* ─── Owned & dedicated — the entity-list names ───────────────────────
     Cedar Mills is the one dedicated plant on the loaded book. The three
     entities after it are legacy sourcing vehicles still on the corporate
     entity list as of Jan 2026 — whether they are dormant shells or still
     consolidate off the published list is one of the genuinely open
  ...
  S({
    id: "SUP-07",
    name: "Harbourview Consolidation",
    own: true,
    site: "Hong Kong",
    country: "Hong Kong",
    region: "South China",
    ...
    scoreLines: LINES(72, 78, 80, 55, 50, [
      ...
      "Legacy sourcing vehicle, undisclosed utilisation — the risk is opacity",
    ...
    openPos: 2,
    openPoValue: 310_000,
    note: "Still on the entity list; not on the published supplier list. Dormant shell or active consolidator is an open question — reliability is marked low for that reason, not for performance.",
  }),
  S({
    id: "SUP-08",
    name: "Harbourview Shenzhen",
    own: true,
    site: "Shenzhen",
    country: "China",
    region: "South China",
    ...
    openPos: 2,
    openPoValue: 260_000,
    note: "The mainland half of the Harbourview pair. Same entity-list question.",
  }),
  S({
    id: "SUP-09",
    ...
    scoreLines: LINES(70, 74, 78, 52, 50, [
      ...
      "Utilisation undisclosed since the sourcing arm consolidated the buy",
      "Internal — no commercial terms",
    ]),
    status: "exit-planned",
    contractExpiry: null,
    openPos: 1,
    openPoValue: 90_000,
    note: "The third legacy vehicle. If the sourcing-arm consolidation finishes anywhere, it finishes here.",
  }),
...
export const PLAYS: Play[] = [
  {
    id: "OPP-001",
    title: "Section 301 relief, HH book",
    kind: "index-hedge",
    stage: "surfaced",
    ...
    basis: "evidence",
    summary:
      "Chapter 69 taxes stoneware by subheading and the Section 301 overlay rides on top of the base rate · the HH dinnerware entries run under a List 3 line at +25% while a granted exclusion covers the identical reactive-glaze subheading, and 19 CFR 141.89 already requires the ceramics detail on the invoice to claim it. The stoneware-heavy HH entries are carrying overlay duty the spec never needed.",
    action: "+25% on one overlay line. Rework the HH entries.",
    evidence: [
      { claim: "$48.2M of HH entries carried the List 3 +25% overlay over 12 months", source: "Broker entry summaries · CBP ACE" },
  ...
  },
  {
    ...
    basis: "mixed",
    summary:
      "Every snack extension quoted outside carries a co-man margin the dedicated lines were built to capture · Cedar Mills is the one dedicated plant, it bakes and packs, and it runs one shift. A second shift brings the bar and cluster extensions in-house without touching the outside book.",
    action: "Margin-stacked outside. Cedar Mills runs one shift — cost the second.",
    evidence: [
      ...
      { claim: "Outside quotes stack 18% co-man margin over the dedicated-line cost model", source: "Should-cost model · FY26" },
    ],
    risks: [
  ...
  {
    id: "OPP-003",
    title: "First-sale valuation, sourcing arm",
    kind: "rfp",
    stage: "surfaced",
    category: "Home & kitchen (FOB)",
    subCategory: "Stoneware & ceramics",
    ...
    basis: "benchmark",
    summary:
      "Target Sourcing Services takes title at origin and appears as shipper of record on the US entries · a first-sale program values duty on the factory invoice rather than the sourcing-arm resale, and every overlay assessed on entered value shrinks with the base. The structure is already in place — the documentation program is not.",
    action: "Title already passes twice. File first-sale on the factory price.",
    evidence: [
      { claim: "Target Sourcing Services is shipper/exporter of record on 2,053 US shipments", source: "US customs manifests" },
    ...
    risks: [
      "Related-party pricing between the sourcing arm and the factories must prove arm's length",
  ...
  },
  {
    ...
    basis: "mixed",
    summary:
      "Set launches fly at $0.75–2.25 a unit and that is right for a reset date — it is not right for steady replenishment of carry-over styles · moving the flat half of the book to ocean holds the calendar and returns the premium. The reset waves stay on air, which is the whole point of having both programs.",
    action: "Resets fly, carry-over sails. Split the standing programs.",
    evidence: [
      { claim: "Air premium runs $0.75–2.25 a unit, 1–4% of retail", source: "Freight cost model" },
      { claim: "TSS AIR and TSS OCEAN run as standing parallel programs", source: "US customs manifests" },
      { claim: "Carry-over styles show flat weekly demand across two sets", source: "OMS · demand history" },
    ],
    risks: [
  ...
  {
    id: "OPP-108",
    title: "Packaging film, annual consignment buy",
    kind: "tail",
    stage: "committed",
    category: "Fixtures & packaging",
    region: "US Midwest",
    ...
    basis: "mixed",
    summary:
      "The freshness promise is a packaging obligation with a known shape · consolidating the film, pouch and label buy onto one annual order against the bake calendar replaces twelve spot buys and holds the $15.6M consigned packaging inventory to plan.",
    action: "Twelve spot buys, one calendar. Commit the annual order.",
    evidence: [
      { claim: "Packaging film bought on 12 spot POs last year", source: "OMS · PO history" },
      { claim: "Bake-calendar model sizes film demand by SKU and date code", source: "Bake planning · River Falls" },
    ],
    risks: [
      "An annual buy mis-sized against the calendar becomes E&O with a printed-film memory",
    ],
    supplierIds: ["SUP-12"],
    effortWeeks: 9,
    owner: "Marcus Whitfield",
    ...
    milestones: [
      ...
      { id: "m4", label: "Legacy spot vendors deactivated", status: "pending" },
    ],
    drift: {
      flagged: true,
      note: "Q3 landed $44K against a $60K ramp — three label variants have not migrated and are still bought spot.",
    },
    events: [
  ...
  {
    id: "OPP-111",
    title: "Softlines, single-country award",
    kind: "consolidation",
    stage: "dismissed",
    ...
    basis: "benchmark",
    summary:
      "Consolidating softlines onto Cambodia priced 5% under the split book · dismissed because the overlay regime moves by country, and a single-origin book turns the next Section 301 list into a single point of failure. The split IS the hedge.",
