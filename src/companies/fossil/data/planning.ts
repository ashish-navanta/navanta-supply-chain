/* ═══════════════════════════════════════════════════════════════
 *  Planning — the IRIS model, on Fossil's book
 *
 ...
 * same Bradshaw cost $48 in Dallas and $86 in Eggstätt — invisible while
 * nothing printed a unit price, and nonsense the moment the overstock table
 * started valuing excess.
 *
 * $40–120, which is where a licensed fashion watch's wholesale cost actually
 * sits — the retail keystone lands it at $99–295 on the ticket.
 */
export function cartonCost(sku: string): number {
  return 40 + Math.round(hash01(`cost|${sku}`) * 80);
}

/* The two distribution centres, from the catalogue rather than a list of their
   own. The fixtures used to name twelve invented sites — "Nashville DC-21",
   "Bakersfield DC-40" — which is not a scope anybody can hold in their head and
   was inventing geography besides. See DCS. */
...
function generated(): SkuPolicyRow[] {
  ...
  /* Per COLOURWAY, not per style. A position is a SKU at a centre — that is
     ...
     ninety-nine. It also kept the book populated when the twelve invented
     branches folded down to two real centres: eight styles across two centres
     is sixteen positions, which is not a book. */
...
export const POSITIONS: SkuPolicyRow[] = (() => {
  ...
  /* A seeded row wins its sku@branch outright. The grid generates a position for
     ...
     one — SKU MK5605-5605 at Qi Guang Watch, say — the grid made a second with the
 ...
 *
 * Fossil runs two in this prototype, so "somewhere else" has exactly one answer
 * and a long position always has a destination. Written as a lookup rather than
 * an index flip so a third centre is a data change, not a logic change.
 */
...
/**
 * How much of Fossil's inventory these DC positions actually represent.
 *
 ...
 *
 *   · IN-TRANSIT. Everything except watches sails, and an ocean leg is weeks
 *     long, so a large block of owned stock is on the water at any moment.
 *   · RETAIL. Around 190 doors carry their own stock, none of it a DC position.
 *   · SERVICE PARTS. The 11-year warranty funds a long-tail parts holding that
 *     the 10-K ties to after-sales service rather than to production.
 ...
 * fashion-accessory business does and which read as a triumph (+71% vs plan)
 ...
 *
 * Lands near 6× for the loaded book. Worth knowing that Fossil's real reported
 * turns are lower still — a business holding this much seasonal, licensed
 * inventory runs closer to 2–3× — so this is the optimistic end of plausible
 * rather than a figure read off a filing.
 */
  ...
  .map((p) => {
    ...
    return {
      ...
      /* Two different moves, and they leave by different doors.
         ...
         buying places it, so "Request 193 from Qi Guang Watch" rather than "Raise
         193 on" — a planner cannot commit Fossil to a purchase.
