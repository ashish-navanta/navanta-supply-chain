...
export function deckFor(e: Exception): DemandDeck {
  ...
  return {
    exception: e,
    summary: short
      ? `${e.sku} at ${e.branch} holds ${e.onHand} units against a ${target}-carton target — ${cover} days of cover on a ${e.leadTimeDays}-day lead from ${e.vendor}. ${e.incoming ? `${e.incoming} units are already travelling and close part of the gap.` : "Nothing is on order."} I recommend requesting ${e.requestedQty} to buying.`
