...
export function agentTaskFor(
  ...
  switch (row.signal) {
      ...
      return {
        ...
        steps: [
          ...
          },
          {
            ...
            text: `Raising the level does not fill a shelf today. Renley Watch Mfg is ${bridge} ${row.qtyUnit} over target on a matching batch and can release the same day, which holds this node while the new level builds.`,
        ...
        outcome: {
          ...
          lines: [
            `Sized on the ${cover?.leadDays ?? 0}-day lead time we accepted, and Renley Watch Mfg covers ${row.party} while the level builds.`,
          ],
    ...
    case "alternate-accepted": {
      ...
      /* "Bradshaw Chronograph 43 · Maple Frost" → "Maple Frost". The swap is
         a finish of the same watch, which is what the order's own `said` field
         describes — so the call talks about colour rather than construction.
         Construction is case, movement and band, and on the same watch in a
        ...
        steps: [
          {
            ...
            text: `Same movement and case, same contract price on ${row.qtyValue} ${row.qtyUnit}, and enough on hand to ship the whole order rather than split it. The floor-set on ${install} does not move.`,
          ...
          },
          {
            ...
            text: `Offered ${colourway} at ${alternateEta} against waiting for the original on ${proposal?.date ?? install}. They chose to wait — the floor-set crew is not in until then, and they would rather not brief them on a different finish.`,
            source: who.phone,
            call: {
              with: `${who.name} · ${row.party}`,
              duration: "0:45",
              /* NO AUDIO, deliberately. The recording on disk is the Shaw
                 ...
                 transcript and drops the player until a Fossil take is
              ...
              turns: [
                ...
                { speaker: first, text: `Yes.` },
                {
                  ...
                  text: `Hi ${first}, this is Christy from Fossil. I'm calling about order ${row.ref}. There are two options. The original order keeps the finish you specified and delivers ${spokenDate(proposal?.date ?? install)}. Or we have the same watch in ${colourway} in stock at ${proposal?.at ?? "the DC"}, which would deliver ${spokenDate(alternateEta)}. Which would you prefer?`,
                },
                { speaker: first, text: `Same watch, just the finish?` },
                {
                  speaker: "Christy",
                  text: `Same watch, same movement, same contract price. The only difference is the dial finish.`,
                },
    ...
    case "pickup-window": {
       ...
       * row uses the templated transcript until a Fossil take is recorded;
      ...
      const recordedTurns: CallTurn[] = [
        ...
        {
          speaker: "Tova",
          text: `Hi Greg, this is Tova from Fossil. I'm calling about order SO-4390, arriving September 6th between 8:00 and 11:00 AM. Does that window work for you?`,
        },
