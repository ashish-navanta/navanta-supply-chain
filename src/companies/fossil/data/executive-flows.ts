 ...
 * write-down depending on what happens next, which is why it belongs at the seat
...
function readFor(tower: ExecTower): TowerRead {
  ...
  if (tower === "balance") {
    ...
    return {
      title: "Under-stock and over-stock, by centre",
      ...
      steps: [
        ...
        },
        {
          ...
          text: `${money(longTotal)} of capital sits above target, worst at ${worstLong.branch}. A centre usually appears on both lists: short on what sells and long on what does not.`,
          source: "Planning · positions over target",
        },
      ],
      lines: [
        "Short is revenue that cannot be served; long is capital that cannot be spent. Same arithmetic, opposite consequence.",
      ],
      tiles: [
        { label: "Short", value: money(shortTotal), tone: "behind" },
        { label: "Long", value: money(longTotal), tone: "behind" },
        { label: "Worst centre", value: worstShort.branch },
      ],
      continueLink: { label: "Open Inventory Planning", href: "/planning/parts" },
      prompts: [
