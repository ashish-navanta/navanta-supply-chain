/* ═══════════════════════════════════════════════════════════════
 *  Target — exception queues
 *
 *  The order the workshop follows: 24,000 units of a partner-brand
 *  throw blanket against a committed store-set date, hit by a
 *  GEOPOLITICAL LEAD-TIME INCREASE — the Dongguan supplier's lead
 *  time jumps 10 days for the next three months — and arriving with
 *  CRUSHED CARTONS in the de-consolidation.
 *
...
/* The vocabulary a planner or a logistics coordinator already uses, rather
   ...
   the second is an in-transit exception, and both are terms a Target reader can
...
export interface InsightCtx {
  ...
  count?: number;
  /** Counterparty is Target-operated. */
  own?: boolean;
...
export function insightText(signal: Signal, ctx: InsightCtx = {}): string {
  switch (signal) {
    ...
    case "alternate-accepted":
      /* Two moves, one answer: the substitute that saves the store set and the date
         ...
         is a category and HH5605-5799 is a decision. */
      ...
      return ctx.sku
        ...
        : `Alternate accepted · store-set date holds, confirm the swap`;
    ...
    case "damage":
      if (ctx.state === "decide" && ctx.units)
        return `${ctx.units ?? 48} units damaged in the de-consolidation · photos in, charge carrier`;
    ...
    case "backhaul":
      return `Co-loading the Yantian consolidation saves ${usd(ctx.amount ?? 0)} · book space`;
    ...
    case "alternate-shipped":
      return `Alternate accepted · shipped ${ctx.date ?? "on time"}, store-set date held`;
...
/**
 * Claim facts for a service row. A damage claim is settled from records Target
...
export interface ActionRow {
   ...
   * differently ("Stoneware Dinnerware Set 16pc" against "Hardlines ·
   * Kitchen"). Named explicitly, the chain is navigable instead of inferred.
   */
  chainFrom?: string;
 ...
 * line breakdown used to hash its own, so the same throw blanket was
 ...
 *
 * The book these rows describe is Target's own — the home and grocery book out
 * of Hearth & Hand, Good & Gather and Threshold — so the styles map onto real
 * style numbers and the SKUs become real ordering references: HH5605-5605 is the
 * Stoneware Dinnerware Set in Cream, and it says so because the catalogue says so.
 *
 ...
 * not answer which glaze, what case pack it ships in, or which supplier makes
 * it — so every surface that wanted any of that invented it locally.
 */
const CATALOGUE_STYLE: Record<string, string> = {
  "Stoneware Dinnerware Set 16pc": "HH5605",
  "Chunky Knit Throw Blanket": "HH7108",
  "Acacia Serving Board": "HH2980",
  "Ceramic Bud Vase 8in": "HH3192",
  "Organic Granola Clusters 12oz": "GG4735",
  "Performance Bath Towel": "TH3184",
  "Cold Brew Concentrate 32oz": "GG3843",
  "Quick-Dry Bath Rug": "TH9204",
};
...
export function linesFor(row: ActionRow): RowLine[] {
  ...
  return weights.map((w, i) => {
    ...
    /* The line's name comes off the SKU, not from a parallel list of colourway
       words. Those two had already drifted: the number said HH5605-5605, which
       is Cream, while the label beside it said Walnut Ember — a made-up
       colourway from a made-up axis, printed against a real reference. One
       lookup, so a line cannot name a glaze its own SKU does not have. */
...
const BUYER: ActionRow[] = [
  ...
  R({
    ...
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Chunky Knit Throw Blanket",
    qtyValue: "24000", qtyUnit: "units", date: "22 Aug", committedOn: "12 Aug",
  ...
  R({
    ...
    party: "Vinh Phat Textiles", partyOwn: false,
    product: "Quick-Dry Bath Rug",
    qtyValue: "43200", qtyUnit: "units", date: "30 Aug", committedOn: "20 Aug",
    status: "Lead time +10d",
    signal: "lead-time-jump", value: 198200, action: "Decide",
  }, { days: 10, months: 3 }),

  R({
    id: "b3", state: "decide", ref: "PO-4463", refSub: "Kiln capacity capped",
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Stoneware Dinnerware Set 16pc",
    qtyValue: "19200", qtyUnit: "units", date: "2 Sep", committedOn: "23 Aug",
  ...
  R({
    ...
    party: "Vinh Phat Textiles", partyOwn: false,
    product: "Performance Bath Towel",
    qtyValue: "25200", qtyUnit: "units", date: "3d ago", committedOn: "20 Aug",
    status: "No reply",
  ...
  R({
    ...
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Chunky Knit Throw Blanket",
    qtyValue: "24000", qtyUnit: "units", date: "2d ago", committedOn: "27 Aug",
  ...
  R({
    ...
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Ceramic Bud Vase 8in",
    qtyValue: "37000", qtyUnit: "units", date: "2d ago", committedOn: "24 Aug",
    status: "No reply",
  ...
  R({
    ...
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Chunky Knit Throw Blanket",
    qtyValue: "19200", qtyUnit: "units", date: "12 Aug", committedOn: "2 Aug",
  ...
  R({
    ...
    party: "Cedar Mills Co-Pack", partyOwn: true,
    product: "Cold Brew Concentrate 32oz",
    qtyValue: "42000", qtyUnit: "units", date: "6 Aug", committedOn: "27 Jul",
    status: "Lead time accepted",
  ...
  R({
    ...
    party: "Luen Hing Housewares", partyOwn: false,
    product: "Stoneware Dinnerware Set 16pc",
    qtyValue: "19200", qtyUnit: "units", date: "28 Jul", committedOn: "18 Jul",
    status: "Overridden",
 ...
 * HH5605-5605 showed 2,400 units while PO-4463 — the same style, the order this
 * whole walkthrough follows — has 6,400 units of that SKU inbound. Two seats
...
const PLANNER: ActionRow[] = [
  /* Every open row is the same exception: the buyer accepted a longer lead time,
     ...
     that has to move. */
  R({
    /* The downstream half of PO-4463. The kiln line's 32 → 42 days is what makes
       the standing buffer too little: safety stock is a lead time expressed as a
       quantity, so when the lead time moves the buffer has to. */
    id: "p1", state: "decide", ref: "Stoneware Dinnerware Set 16pc", refSub: "SKU HH5605-5605",
    party: "Woodland RDC", partyOwn: true,
    product: "Stoneware Dinnerware Set 16pc",
    ...
    cover: coverFor({
      ...
      moq: 96, classification: "AZ", supplier: "Luen Hing Housewares",
      incoming: incomingFor("HH5605-5605"),
    }),
  }, { upstream: "PO-4463" }),

  R({
    id: "p2", state: "decide", ref: "Chunky Knit Throw Blanket", refSub: "SKU HH7108-7110",
    party: "Woodland RDC", partyOwn: true,
    product: "Chunky Knit Throw Blanket",
    ...
    cover: coverFor({
      ...
      moq: 144, classification: "AY", supplier: "Luen Hing Housewares",
      incoming: incomingFor("HH7108-7110"),
    }),
  }),

  R({
    id: "p3", state: "decide", ref: "Organic Granola Clusters 12oz", refSub: "SKU GG4735-4735",
    party: "Cedar Falls RDC", partyOwn: true,
    product: "Organic Granola Clusters 12oz",
    ...
    cover: coverFor({
      ...
      moq: 144, classification: "AZ", supplier: "Cedar Mills Co-Pack",
      incoming: incomingFor("GG4735-4735"),
    }),
  }),

  R({
    id: "p4", state: "decide", ref: "Performance Bath Towel", refSub: "SKU TH3184-3184",
    party: "Woodland RDC", partyOwn: true,
    product: "Performance Bath Towel",
    ...
    cover: coverFor({
      ...
      moq: 96, classification: "BY", supplier: "Vinh Phat Textiles",
      incoming: incomingFor("TH3184-3184"),
    }),
  }),

  R({
    id: "p5", state: "decide", ref: "Quick-Dry Bath Rug", refSub: "SKU TH9204-9204",
    party: "Cedar Falls RDC", partyOwn: true,
    product: "Quick-Dry Bath Rug",
    ...
    cover: coverFor({
      ...
      moq: 72, classification: "BZ", supplier: "Vinh Phat Textiles",
      incoming: incomingFor("TH9204-9204"),
    }),
  }),

  R({
    id: "p6", state: "decide", ref: "Cold Brew Concentrate 32oz", refSub: "SKU GG3843-3843",
    party: "Woodland RDC", partyOwn: true,
    product: "Cold Brew Concentrate 32oz",
    ...
    cover: coverFor({
      ...
      moq: 96, classification: "CY", supplier: "Cedar Mills Co-Pack",
      incoming: incomingFor("GG3843-3843"),
    }),
  }),

  R({
    id: "p7", state: "settled", ref: "Stoneware Dinnerware Set 16pc", refSub: "SKU HH5605-5799",
    party: "Woodland RDC", partyOwn: true,
    product: "Stoneware Dinnerware Set 16pc",
    ...
    cover: coverFor({
      ...
      moq: 96, classification: "CZ", supplier: "Luen Hing Housewares",
      incoming: incomingFor("HH5605-5799"),
    }),
  }, { units: 624 }),
];
...
const CSR: ActionRow[] = [
  R({
    /* The account end of PO-4471. Same throw blanket, same ten days — 2,880 of
       the 24,000 units on that PO are this account's, and the buying seat is
       holding the supplier side of this exact move. */
    id: "c1", state: "decide", ref: "SO-4471", refSub: "Geopolitical lead-time increase · store-set booked",
  ...
  R({
    id: "c2", state: "decide", ref: "CLM-2041", refSub: "Six cartons crushed · photos in",
  ...
  R({
    /* Took the alternate rather than the wait — the flavour was not specified,
       so keeping the store-set date was worth more than keeping the variant.
       Grocery, which is why the count is cases-of-twelve deep: 14,400 pouches
       is 1,200 cases, and at $4.25 a pouch that is the $61,200 on the row. */
    id: "c3", state: "decide", ref: "SO-4488", refSub: "Accepted the alternate flavour",
    party: "Blue Ridge Jewelers", partyOwn: false,
    product: "Organic Granola Clusters 12oz",
    qtyValue: "14400", qtyUnit: "units", date: "22 Aug",
    status: "Account answered",
  ...
  R({
    /* Both halves of a good outcome, on one line: the account has taken the
       revised date in writing AND accepted an alternate glaze for part of the
       order, so 288 of the 1,536 sets stop waiting on Luen Hing's kiln
       altogether. What is left for this desk is confirming the substitution
       against the store set — the date itself is no longer in dispute, which is
       why the row no longer asks anyone to negotiate one. */
    ...
    chainFrom: "PO-4463",
  }, { sku: "HH5605-5799", units: 288, date: "6 Sep", days: 4 }),
  ...
  R({
    /* The alternate came off a different purchase order — that is what taking an
       ...
       order this date now depends on, and the answer is Luen Hing Housewares'.
       Serveware, so the count runs deeper than the dinnerware rows: 4,608 boards
       at roughly $25.70 is the $118,600 on the row. */
    id: "c6", state: "settled", ref: "SO-4390", refSub: "Alternate accepted",
    chainFrom: "PO-4402",
    party: "Peachtree Jewelers", partyOwn: false,
    product: "Acacia Serving Board",
    qtyValue: "4608", qtyUnit: "units", date: "26 Aug",
...
const LOGISTICS: ActionRow[] = [
  R({
    /* SO-4390, not SO-4471 — the order record already said so and was being
       contradicted here. Its PRO number is 70412, its carrier is dedicated
       tractor #218 out of the Target fleet and its lane is Woodland RDC →
       Atlanta: this load, named three different ways. Two loads asserting the
       same order left the queue showing one 4,608-board sales order being
       carried twice, at full value both times. */
    id: "l1", state: "decide", ref: "LD-70412", refSub: "Woodland RDC → Atlanta, GA",
    chainFrom: "SO-4390",
    party: "Dedicated · #218", partyOwn: true,
    product: "Acacia Serving Board",
    qtyValue: "4608", qtyUnit: "units", date: "Today 14:20",
    status: "ETA reconciled",
    signal: "eta-conflict", value: 118600, action: "Confirm ETA",
  }, { sources: 3, days: 6 }),

  R({
    /* The one inbound move on the book: Vinh Phat Textiles' bath rugs have
       cleared the terminal at Long Beach and have to move inland to Woodland —
       the first RDC behind the gateway, which is why the chassis and the gate
       slot are the row's whole problem. It is tied to PO-4488 rather than to an
       account order because that is what it is carrying — the column reads the
       reference and says inbound, which is the fact that changes who works the
       row and against what clock. Same 43,200 units as the PO, because the
       boxes are the PO. */
    id: "l2", state: "decide", ref: "LD-70455", refSub: "Long Beach → Woodland RDC",
    chainFrom: "PO-4488",
    party: "Forwarder · Kuehne+Nagel", partyOwn: false,
    product: "Quick-Dry Bath Rug",
    qtyValue: "43200", qtyUnit: "units", date: "Tomorrow 09:00",
  ...
  R({
    /* The outbound leg of SO-4463 — the order this whole demo follows, from
       Luen Hing's capped kiln line through Iris's stocking policy to Christy's
       re-promise. Every figure on the row is the order's own: 1,536 dinnerware
       sets, $88,400, the dedicated fleet down Woodland RDC → Salt Lake City,
       arriving on the 6 Sep the account was re-promised. Which is also why the
       row is here: Summit's crew is booked for the day the sets land, so there
       is no slack left at all — the delivery appointment is the thing to nail
       down before the date moves again. */
    id: "l3", state: "decide", ref: "LD-70398", refSub: "Woodland RDC → Salt Lake City, UT",
  ...
  R({
    /* The end of the same thread: the load that finally carried SO-4463 arrived
       with 48 sets crushed — stoneware ships two sets to a foam-cell carton, so
       that is two dozen cartons off a pallet — and what began as a kiln's
       capacity problem finishes as a return to book and a credit to raise. */
  ...
  R({
    /* Its own order, not SO-4463. Two loads against one sales order — a
       48-unit damage claim and a 21,120-bottle recovery — described a shipment
       that cannot exist: nobody recovers a reefer load to replace 48 crushed
       cartons. SO-4436 is already in transit and already at risk with a purchased
       carrier, which is exactly the order a dedicated-fleet expedite is costed
       against — and it is cold brew, dated and refrigerated, so the recovery
       is a reefer unit and the clock is the shelf life, not the promise. */
    id: "l5", state: "decide", ref: "LD-70460", refSub: "Recovery costed",
    chainFrom: "SO-4436",
    party: "Target Fleet · Reefer expedite", partyOwn: true,
    product: "Cold Brew Concentrate 32oz",
    qtyValue: "21120", qtyUnit: "units", date: "Today 16:00",
    status: "Recovery ready",
    signal: "recovery", value: 104200, action: "Dispatch",
  }, { amount: 2400 }),

  R({
    /* A co-load is not a floating cost line — it is space on a box that is
       about to sail. Naming the order is what lets the row say which
       consolidation is about to run light: the Yantian box that feeds SO-4402
       cuts off this afternoon with 1,968 throws of cube still open, and the
       $3,800 the sourcing office quoted for co-loading it is unbooked. */
    id: "l6", state: "decide", ref: "LD-70471", refSub: "Yantian consolidation → Woodland RDC",
    chainFrom: "SO-4402",
    party: "Target Sourcing Services · Ocean", partyOwn: true,
    product: "Chunky Knit Throw Blanket",
  ...
  R({
    id: "l7", state: "settled", ref: "LD-70470", refSub: "Woodland RDC → Charlotte, NC",
    chainFrom: "SO-4377",
    party: "Dedicated · #331", partyOwn: true,
    product: "Stoneware Dinnerware Set 16pc",
    qtyValue: "1296", qtyUnit: "units", date: "Today 06:40",
    status: "Dispatched",
    signal: "dispatched", value: 74300, action: "Track",
  }),

  R({
    /* The import leg that fed SO-4547, delivered clean: Luen Hing's vases,
       consolidated at Yantian, landed at LA/Long Beach and de-consolidated at
       Woodland. The same product as PO-4547 on the buying desk, because the
       load is that PO's goods on the water. */
    id: "l8", state: "settled", ref: "LD-70350", refSub: "Yantian consolidation → Woodland RDC",
    chainFrom: "SO-4547",
    party: "Target Sourcing Services · Ocean", partyOwn: true,
    product: "Ceramic Bud Vase 8in",
    qtyValue: "5760", qtyUnit: "units", date: "5 Aug",
    status: "Delivered",
...
const CONTACTS: Record<string, PartyContact> = {
  /* One contact per supplier — and WHOSE person they are follows ownership.
     Luen Hing and Vinh Phat are independent contract suppliers, so the card is
     their production merchandiser: the person who actually answers when a
     buyer chases a line. Cedar Mills runs dedicated co-pack lines against
     Target's own recipes, so the card is a Target co-manufacturing scheduler
     on a target.com address. An earlier build had this the other way round
     because that company owned its plants; carrying that over would have put
     Target employees inside factories Target does not own. */
  "Luen Hing Housewares": {
    name: "Kenny Cheung", role: "Production merchandiser",
    phone: "+86 769 5550 129", email: "k.cheung@luenhinghousewares.com",
    hours: "Dongguan · on site 8am–6pm CST", respondsIn: "Answers WeChat and phone same shift", prefers: "call",
  },
  "Vinh Phat Textiles": {
    name: "Linh Tran", role: "Production planner",
    phone: "+84 28 5550 177", email: "l.tran@vinhphattextiles.com",
    hours: "Ho Chi Minh City · on site 8am–5pm ICT", respondsIn: "Answers same shift", prefers: "call",
  },
  "Cedar Mills Co-Pack": {
    name: "Arjun Mehta", role: "Co-manufacturing scheduling manager",
    phone: "+1 (715) 555-0042", email: "a.mehta@target.com",
    hours: "River Falls, WI · on site 7am–4pm CT", respondsIn: "Replies to email same day", prefers: "email",
  },
  ...
  switch (label) {
    ...
    case "Follow up":
      return [
        ...
        "The store-set crew books this week; after that the choice narrows on its own.",
        "If they're silent because the set slipped, capture the new store-set date.",
      ];
    ...
    entries.push(
      {
        ...
        what:
          row.signal === "capacity"
            ? `${row.party} confirmed kiln capacity is committed through the quarter — ${requested} days for the next three months`
      ...
      },
      {
        ...
        source: `${agent} · OMS, Demand planning`,
      },
      {
        when: "2 days ago",
...
export interface ThreadEntry {
  ...
  kind: "email" | "call";
  /** True when Target sent it or placed it — drives the icon. */
  outbound: boolean;
  from: string;
  to: string;
  when: string;
...
export function threadFor(row: ActionRow, agent: string): ThreadEntry[] {
  ...
  const us = row.partyOwn ? "planning@target.com" : "sourcing@target.com";
  const cause = causeOf(row.signal);
  ...
  if (cause === "supply") {
    ...
    return [
      opening,
      {
        ...
        body:
          row.signal === "capacity"
            ? `Kiln capacity is committed through the quarter. We can hold ${row.qtyValue} ${row.qtyUnit} but not before the revised date — assume ten days beyond the original for the next three months.`
      ...
      },
      {
        ...
        kind: "email",
        outbound: true,
        from: `${agent.toLowerCase()}@target.com`,
        to: "service@target.com",
        when: "2 days ago",
  ...
  if (cause === "awaiting") {
    return [
      opening,
      {
        ...
        body: `Following up: we still have no acknowledgement on ${row.ref}. Please confirm a firm ship date against the committed store-set date.`,
