...
/** One account order, in full. Every SO- reference in this seat lands here. */
...
export const NAV: Record<Persona, NavGroup[]> = {
  ...
  csr: [
    ...
    },
    {
      ...
      entries: [
        ...
        /* The book, on the seat that looks things up in it. It sat under
           Planning on the reasoning that the planner owns what Target stocks —
           ...
           service rep who is asked "what does that colourway look like, and is
           there anything close to it" while an account waits on the line, which
        ...
        /* A grid, not a cube. The rail carries Package directly above this one
           ...
           book this page holds is shelf product in colourway grids, so GridFour
           is what the catalogue actually looks like on the page: a planogram of
           swatches, not a carton. */
    ...
    {
      /* The account, on its own band. It sat under Resolve with Claims on the
         ...
         account book is the account behind every order and every claim, and it
         is read when nothing is wrong at all. Grouped together the rail said the
         only reason to open an account is that they have complained. */
      label: "Accounts",
 ...
 * purchase order names the account waiting on it, a claim names both — and every
