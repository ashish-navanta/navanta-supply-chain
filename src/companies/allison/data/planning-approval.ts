 ...
 *
 *  The buying seat's tasks are AUTHORED — a capped line at Cline Tool, a
 ...
 *
 *  Inventory Planning has 198 positions, and there is no writing them.
...
import { excessOf, isShort, otherDc, targetStock, type Exception } from "./planning";

/* No reference number here, deliberately. A stores requisition is raised by
   the buying desk and a stock transfer is cut by receiving — the planner
   approves a quantity, and numbering it REQ-4371 in this transcript claimed a
   document that does not exist yet and would carry a different number when it
   did. */
...
export function approvalTaskFor(e: Exception): AgentTask {
  ...
  return {
    ...
    /* One statement of the move. It used to append the recommendation
       lower-cased, which said the quantity twice and turned Kirby Risk Supply Co
       into "kirby risk" — "Approve 67 units — request 67 from cline tool on
       AL5605-5605". The vendor is a proper noun and the figure only needs saying
       once. */
    ask: short
    ...
    steps: [
      {
        ...
        text: `${e.onHand} on hand, ${e.incoming} inbound, ${e.demandMean.toFixed(1)} a day against a ${target}-unit target. Unchanged since the deck was drawn.`,
      ...
      },
      {
        ...
        text: short
          ? `Approved as demand, suggested source ${e.vendor}. Buying raises the stores requisition and places it.`
          : `Approved to move. Receiving cuts the stock transfer and books the shuttle.`,
    ...
    outcome: {
      ...
      /* One line. The case was made in the deck; this says what happens next
         ...
         "which meets its 443-unit target" — which is true by construction,
      ...
      lines: [
        short
          ...
          : `${to} takes it without a new order. Nothing moves until receiving books the shuttle.`,
      ],
  ...
  return [
    ...
    },
    {
      ...
      /* The one gate that can fail on a row that still auto-routed, and it does
         on three of the fifty-five: the grid decides routing on confidence and
  ...
  return {
    ...
    steps: [
      ...
      },
      {
        ...
        text: short
          ? `${e.onHand} on hand and ${e.incoming} inbound against a ${target}-unit target — ${gap} short, so ${e.requestedQty} to ${e.vendor}.`
          : `${e.onHand} on hand against a ${target}-unit target — ${gap} over, so ${e.requestedQty} to ${e.transferTo ?? otherDc(e.branch)}.`,
        source: `Stocking policy · ${e.sku}@${e.branch}`,
      },
    ],
    outcome: {
    ...
    steps: [
      ...
      },
      {
        label: short
          ...
          : `Send the transfer to logistics — ${e.branch} to ${to}`,
        text: short
          ? `Overridden as demand, suggested source ${e.vendor}. Buying raises the stores requisition and places it.`
          : `Overridden to move. Receiving cuts the stock transfer and books the shuttle.`,
        source: short ? "Buying · requisition queue" : "Logistics · transfer queue",
      },
    ],
    outcome: {
