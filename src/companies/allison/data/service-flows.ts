 ...
 * had no agent surface at all: the requisition book, the claim book, and the
 * maintenance-team book. Each row gets one move, named for what it actually
 * does, and the run lands an artifact — a re-promise note, an adjudication, a
 * service-recovery letter — rather than a sentence about one.
 */
import type { AgentTask, FlowArtifact } from "./agent-actions";
import type { Account, ServiceClaim, ServiceOrder } from "./service";
      ...
      text: slipped
        ? `Promised ${o.promisedOn}, the record now believes ${o.currentEta}.` +
          (o.crewBooked && o.installOn
            ? ` A shutdown crew is booked for ${o.installOn} and cannot be moved — that is what the slip actually breaks.`
    ...
    ? {
        ...
        body: [
          o.shortPallets
            ...
            : `Clean delivery. Nothing outstanding on this requisition.`,
        ],
      }
    : {
        kind: "doc",
        ...
        body: slipped
          ? [
              `Your requisition ${o.id} was promised ${o.promisedOn}. The carrier now has it landing ${o.currentEta}.`,
              o.crewBooked && o.installOn
                ? `We know a crew is booked for ${o.installOn}. Tell us whether to hold the date and split the delivery, or move the PM window — we will carry the re-book either way.`
                : `Nothing else on the requisition changes. Let us know if the new date creates a problem on the floor.`,
            ]
          : [
              `Your requisition ${o.id} is on track to land ${o.currentEta}, as promised.`,
    ...
    outcome: {
      ...
      lines: [
        slipped && !o.deliveredOn
          ? `The note is drafted in the plant's own dates. Sending it opens the re-book conversation.`
          : o.deliveredOn
          ? `The file is complete. Nothing is waiting on you.`
          : `The window is confirmed and the plant has the date in writing.`,
      ],
      tiles: [
      ...
      prompts: slipped
        ? [
            "What does the slip cost the plant in machine hours?",
            "Can we split the delivery to hold the shutdown date?",
            "Who else is on this carrier this week?",
          ]
        : [
            "Which requisitions are closest to slipping?",
            "How is this team's on-time rate?",
 ...
 *
 *   un-adjudicated → adjudicate from the requisition and the receipt
  ...
  const steps = [
    {
      ...
      text:
        ...
        `lot ${c.batch}. Opened ${c.openedOn}.`,
      source: `Claim file · ${c.id}`,
    },
    {
      label: "Checked the evidence",
      text:
        `${c.photos} photos on file, receipt ${c.receipt} matched to requisition ${c.orderId}.` +
    ...
    },
    {
      ...
      text: decided
        ...
        : `${usd(c.requested)} asked. Priced off the requisition lines and the receipt, the defensible figure is ${usd(credit)}.`,
    ...
    outcome: {
      ...
      lines: [
        overCap
          ...
          : `${usd(credit)} is inside policy. Approving it credits the team's cost centre and closes the file.`,
      ],
      tiles: [
      ...
      prompts: [
        ...
        `Other claims against lot ${c.batch}?`,
        overCap ? "Who signs above the cap?" : "Can we recover this from the carrier or the supplier?",
      ],
    },
  };
}
...
/**
 * The maintenance-team book had no agent surface at all. The move here is a
 * read of the relationship that ends in a drafted note — the thing a CSR would
 * write after seeing a plant's claim rate climb.
 */
  ...
  const steps = [
    {
      ...
      text:
        ...
        `${usd(d.ytdRevenue)} issued year to date on ${d.paymentTerms}.`,
      source: `Account master · ${d.id}`,
    },
    {
      label: "Read how we serve them",
      text:
        `${d.onTimePct}% delivered on the promise over twelve months, ${d.claimRate} claims per hundred requisitions.` +
        (struggling
          ? " Both sit outside where this tier should be — the team is being served worse than its machines are worth."
    ...
    },
    {
      ...
      text: struggling
        ? `A note that names the misses, says what we are changing, and asks for the meeting — signed by you, not by the stores.`
        : `${d.note}`,
    ...
    outcome: {
      ...
      lines: [
        struggling
          ...
          : `The team is being served where its tier expects. Nothing needs a move today.`,
      ],
      tiles: [
        { label: "YTD issues", value: usd(d.ytdRevenue) },
      ...
      artifact: struggling
        ? {
            ...
            body: [
              `Over the last twelve months we delivered ${d.onTimePct}% of your requisitions on the date we promised, and you raised ${d.claimRate} claims per hundred. For a team of your standing, neither figure is good enough.`,
              `We are putting your requisitions on the earlier shuttle cut-off and holding safety stock at Indy Central Stores on your two fastest-moving parts. You should feel the difference inside a quarter.`,
        ...
        : {
            ...
            columns: ["Measure", "This team", "Tier expects"],
            rows: [
              ...
              { cells: ["Charge-back terms", d.paymentTerms, d.paymentTerms] },
            ],
          },
      prompts: [
        "What is driving the claim rate?",
        "Which requisitions slipped for this team?",
        struggling ? "Escalate to the OEM?" : "Are they a candidate for the onsite crib?",
      ],
    },
  };
}
