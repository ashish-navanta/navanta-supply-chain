 ...
 *  you whether the distributor has even acknowledged the thing.
 *
 ...
 *  a buyer's PO and a maintenance team's stores requisition can both
 *  feed the same card rather than growing two of them.
 * ═══════════════════════════════════════════════════════════════ */
...
/**
 * A purchase order runs four stages, and they are Allison's stages, not the
 * distributor's.
 *
 * A PO is INBOUND — it ends at a plant stores dock, not at a maintenance crew —
 * so the last stage is a receipt, not a delivery. "Delivered" borrowed the
 * outbound language of a stores requisition and pointed the arrow the wrong way.
 *
 * "In production" went with it. Allison does not see inside Cline's shop or the
 * OEM's factory, so a stage named for the vendor's work claimed visibility this
 * seat does not have. Every stage here is something the buyer can actually
 * observe happening: the order goes out, the vendor acknowledges it, the goods
 * ship, they book in at stores. That is also the shape of the outbound stepper
 * on a requisition, which makes the two readable as the two halves of one
 * journey rather than two schemes.
 */
export const PO_STAGES = ["Raised", "Acknowledged", "Shipped", "Received at stores"] as const;
...
function alertFor(row: ActionRow): string | undefined {
  ...
  switch (row.signal) {
    case "lead-time-jump":
      return `${row.refSub} — the supplier has moved the date, and it holds for three months`;
    case "capacity":
      return `${row.refSub} — the allocation is capped, so this cannot simply be expedited`;
    case "silent-po":
...
export function poStateFor(row: ActionRow, contactName?: string): TrackedState {
  ...
  return {
    ...
    /* Received at stores is an arrival, not a state of progress — see the note
       on the outbound run. */
 ...
 * does not move — writing a lead time does not put the parts on a truck —
...
export function poStateCommitted(row: ActionRow, days: number, owed?: string): TrackedState {
  ...
  return {
    ...
    /* The stages do NOT advance. Writing a date to the record does not put the
       parts on a truck — the line is still waiting on the vendor's despatch, and
    ...
    /* The band clears only when the commit is whole. A lead time written to the
       record while the maintenance team still holds the old promise is not
       resolved, and dropping the amber there would be the card saying so. */
    alert: owed,
    etaLabel: "Committed",
    etaLate: false,
 ...
 * maintenance team's requisition does not get "acknowledged" or go
 * "into production" — it is placed, allocated against crib stock,
 * shipped out on a stores truck, and delivered to the crib. Four
 * stages again, and the two the service desk can actually act on are
 * the middle two, which is where every row in that queue sits.
 * ─────────────────────────────────────────────────────────────── */

/**
 * Outbound: it leaves a plant stores and ends at the maintenance team's crib.
 *
 * Read from the order model's own labels rather than written again here. They
 * had drifted — the queue said "Allocated" where the requisition page said "In
 * process", about the same pick on two screens — and a second list of stage
...
function soReachedFor(row: ActionRow): number {
  if (row.state === "settled") return 3;
  switch (row.signal) {
    /* Nothing has shipped: the team is still choosing what they want. */
    case "options-drafted":
    case "awaiting-customer":
      return 1;
    /* Still at stores: the carrier has not been chosen, so nothing has left. */
    case "carrier-choice":
      return 1;
    case "eta-conflict":
    case "pickup-window":
...
function soAlertFor(row: ActionRow): string | undefined {
  ...
  switch (row.signal) {
    case "options-drafted":
      /* The team has chosen; the record has not caught up. That gap is the
         whole reason the row is in front of a person. */
      return `${row.refSub} — the team has chosen to wait, and the promise still reads the old date`;
    case "awaiting-customer":
      return `${row.refSub} — nothing moves until the team picks`;
    case "eta-conflict":
...
/**
 * A stores requisition as the tracking card draws it.
 *
 ...
 * whose outage crew is booked for Thursday.
 */
export function soStateFor(row: ActionRow, contactName?: string): TrackedState {
  ...
  return {
    ...
    /* The last stage, once reached, is DONE — not active. A delivered
       requisition is delivered: there is nothing in progress at the end of a
       run. Marking it active left the final dot painted as the thing still
       owed, which on a row needing a decision came out red, so a clean delivery
       read as a failure. */
...
/**
 * The same requisition once the new date is on the record.
 *
 * The amber band clears because the thing that was wrong — a promise the
 * distributor could not meet — is no longer wrong. The stage does not advance:
 * re-promising does not allocate or ship anything, and pretending otherwise
 * would be the card flattering the work.
 */
 ...
 * card. Null where a row has no journey to draw: a crib min and a stores truck
 * run have no stages, and inventing four for them would be a progress bar that
 * means nothing.
 */
export function stagesFor(row: ActionRow): TrackedStage[] | null {
  switch (row.signal) {
    ...
    /* A stores requisition is outbound whatever fed it. These rows all name the
       purchase order upstream of them, and for a while that reference was being
       read as a direction — which put "Shipped" against a requisition sitting
       at a distributor. What supplied a requisition does not change which way
       it is going. */
    case "options-drafted":
    ...
    /* The logistics seat's rows are deliveries, and a delivery takes the
       direction of what is on it. They used to fall through to null and show
       the cause name in the status column instead — which told the reader what
       kind of problem it was and never where the truck had got to. */
    case "carrier-choice":
    case "pickup-window":
    case "recovery":
    case "backhaul":
    case "dispatched":
      /* An inbound delivery runs the purchase order's track. A Kirby Risk branch
         truck on I-65 out of Lafayette is at "Shipped" on its way to "Received
         at stores"; calling it "In process" borrowed the outbound vocabulary and
         pointed the arrow the wrong way, which is the mistake the PO stepper
         exists to stop. */
      if (row.chainFrom?.startsWith("PO-")) {
        /* The goods have left the vendor and have not booked in: that is the
           whole of what a delivery in Tova's queue can be. */
 ...
 * stage reads as a fact — "Received at stores · 2 Sep" says it arrived — when
 * it is at best a projection and, on a line whose date has just moved, a
 * projection the buyer has not yet agreed to. The expected date belongs in the
 * agent's band, where it is stated as the decision it is.
 */
export function poDates(row: ActionRow, plannedLeadDays: number): Record<string, string> {
  ...
  /* The day the vendor replied — distributors acknowledge within a couple of
     days of release. It is tempting to put the committed date here instead,
     since that is what the current state turns on, but a bare date under
     "Acknowledged" reads as when it was acknowledged; the committed date is
     stated as a commitment in the agent's band, where it cannot be misread as
     an event. A silent PO never reaches this stage and so shows nothing. */
