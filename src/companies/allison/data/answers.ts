...
const ANSWERS: Record<string, () => string> = {
  ...
  "Compare variants on a style": () => {
    const style = CATALOGUE.reduce((a, b) => (b.colourways.length > a.colourways.length ? b : a));
    return `Open a style and I will lay its variants side by side. ${style.name} is the widest on the book at ${style.colourways.length} — the product page draws them all, and the catalogue's image search ranks them against a photograph.`;
  },
  ...
  "What happens if we do nothing?": () => {
    const roll = planningRollup();
    return `${money(roll.dollarsAtRisk)} stays exposed and ${roll.critical} critical positions run down to zero on their current cover. Nothing here recovers on its own — lead times are 24 to 44 days.`;
  },
  ...
  "Why do the ETA feeds disagree?": () => {
    ...
    return `${conflicts} orders carry ETAs that disagree. The carrier reads its own network, the factory reads its schedule, and Fossil reads the load — when they diverge the widest spread is the one worth acting on, not the newest.`;
  },
