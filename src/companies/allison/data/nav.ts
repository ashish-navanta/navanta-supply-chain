...
/** One stores requisition, in full. Every SO- reference in this seat lands here. */
 ...
 * edge to edge. Inventory Planning's exception list is twelve columns; a
 * requisition detail is a 12-column grid whose top row wants eight of them for
 * the stepper. Taking 380px out of either costs the reader the right-hand half
 * of what they came to read, so on these the panel starts collapsed and floats
 * when summoned.
 *
 * The trailing slash on the order route is load-bearing — the requisition BOOK
 * is a normal table and keeps the panel docked beside it; only a single
 * requisition is wide.
 */
const WIDE_PAGES = [
  ...
  /* A single part, for the same reason as a single requisition. The record page
     opens on the part's own detail across the full width and carries a
...
export const NAV: Record<Persona, NavGroup[]> = {
  ...
  planner: [
    ...
    },
    {
      label: "Planning",
      entries: [
        { key: "/planning/parts", label: "Unit Planning", icon: ListBullets },
      ],
    },
    {
      label: "System",
  ...
  /* No command center on this seat. The service desk opens the day on the queue
     and works outward from it — a landing page ahead of that was a page to click
     past. `PERSONAS.csr.route` already points at the action center, so the seat
     guard sends /service there rather than rendering a page the rail cannot
     reach. */
  csr: [
    {
      entries: [
    ...
    {
      /* The pick path. One page: the book, and the requisition behind any line
         in it — where a part is and whether it will land before the PM window
         are the same question, and the reconciled ETA belongs on the
         requisition it is about rather than on a page of its own. */
      label: "Fulfil",
      entries: [
        { key: SERVICE_ROUTES.orders, label: "Orders & releases", icon: Package },
        /* The book, on the seat that looks things up in it. It sat under
           Planning on the reasoning that the planner owns what stores stock —
           true, and not the same as who reads the catalogue. The planner works
           positions: a material at a crib, with a min/max and a buffer. It is
           the service desk who is asked "which manufacturer is that, and is
           there an approved equivalent" while a maintenance supervisor waits on
           the line, which is the question this page answers.
           The other seats still reach it by clicking a SKU. */
        /* A grid, not a cube. The rail carries Package directly above this one
           and two box glyphs in a row are indistinguishable at 20px — and the
           book this page holds is a parts catalogue laid out by family and
           manufacturer, so GridFour is what the page actually looks like. Same
           glyph the "All categories" scope uses, which means the category
           control and the catalogue now agree. */
    ...
    },
    {
      /* Settling money. Kept apart from Fulfil because they are different work:
         one is chasing a delivery, the other is arguing about a dented drum. */
      label: "Resolve",
      entries: [{ key: SERVICE_ROUTES.claims, label: "Claims", icon: ClipboardText }],
    },
    {
      /* The plant team, on its own band. It sat under Resolve with Claims on the
         ...
         plant-team book is the crew behind every requisition and every claim,
         and it is read when nothing is wrong at all. Grouped together the rail
         said the only reason to open a maintenance team is that they have
         complained. */
      label: "Customers",
      entries: [{ key: SERVICE_ROUTES.accounts, label: "Customers & channel", icon: Storefront }],
    },
  ],
  /* Same three-band shape as the other seats, and the bands answer the seat's
     two halves: MOVE is the delivery and the trucks under it — where a thing is
     and whether anything can pull it — and COST is what the moving is worth,
     which is a different question asked at a different time of day. Splitting
     them keeps a rate ledger from reading as a work queue. */
  logistics: [
    {
      entries: [
        {
    ...
    },
    {
      label: "Move",
      entries: [
        { key: LOGISTICS_ROUTES.loads, label: "Shipments", icon: Package },
        { key: LOGISTICS_ROUTES.fleet, label: "Outbound fleet", icon: Truck },
      ],
    },
    {
      label: "Cost",
 ...
 *
 * Records cross seats constantly — a delivery names the requisition it carries,
 * a purchase order names the maintenance team waiting on it, a claim names both
 * — and every one of those links pointed at a route the reader's own rail does
 * not have. The guard refused them and sent the reader home, so the links
 * looked broken when the destination was right and only the seat was wrong.
 *
...
export function seatOwning(pathname: string): Persona | undefined {
  /* The most SPECIFIC claim wins, not the first seat that happens to match.
     ...
     "/planning" — so it claimed /planning/products too, and the parts
