...
import type { Exception, SkuPolicyRow } from "./planning";
...
export function exceptionTaskFor(
  ...
  const outcomeByIntent = {
    approve: {
      ...
      line: `${e.recommendedAction}. The stores requisition is raised against ${e.vendor} and the exposure closes when it lands on the dock.`,
    },
    override: {
  ...
  return {
    ...
    outcome: {
      ...
      prompts: [
        ...
        `Is ${e.branch} the right stores to hold this?`,
        intent === "approve" ? "Can another stores cover it instead?" : "What would the system do here?",
      ],
    },
  };
}
...
/**
 * A SKU's run on the catalogue: sweep every position and name the stores
 * that is actually exposed. The catalogue lists what Allison stocks; this
 * answers the only question a stores planner asks of it.
 */
export function skuTaskFor(
  ...
  const steps = [
    {
      ...
      text: `${sku} sits at ${positions.length} stores with ${total} units on hand and incoming across all of them.`,
      source: "SAP ECC · MM stock",
    },
    {
      ...
      source: worst ? `SAP ECC · MM · ${worst.key}` : "SAP ECC · MM stock",
    },
    {
      label: "Read the rebalance",
      text:
        positions.length > 1 && worst
          ? `The longest stores could cover the shortest without buying anything — a stock transfer inside the network on a matching material is the one write ${agent} is allowed to make here.`
          : `A single stores holds this SKU, so there is nothing to rebalance against.`,
  ...
  return {
    ...
    outcome: {
      kind: "settled",
      title: `${sku} — ${usd(atRisk)} at risk across ${positions.length} stores`,
      lines: [
        worst
          ? `${worst.branch} is the stores to fix first. Everything else on this SKU is holding.`
          : `Nothing on this SKU needs a move today.`,
      ],
      tiles: [
        { label: "Stores", value: String(positions.length) },
      ...
      artifact:
        positions.length > 0
          ? {
              ...
              columns: ["Stores", "On hand", "Safety", "At risk"],
          ...
          : undefined,
      prompts: [
        "Rebalance from the longest stores?",
        "What is driving issues here?",
        "Is this a consolidation candidate?",
      ],
    },
  };
}
