 ...
 *
 * Mirrors `play-drafts.ts` for the Suppliers page. Each vendor gets one
 ...
 */
import type { Supplier, SupplierStatus } from "./buying";
import { SUPPLIER_STATUS_LABEL, money } from "./buying";
import type { AgentTask } from "./agent-actions";
import { BUYING_ROUTES } from "./nav";
 ...
 * branch that fits wins, because a slipping distributor who *also* has a terms
...
export function supplierDraftFor(s: Supplier): SupplierDraft {
  /* Allison-operated sources — the onsite crib and the plant storerooms.
     Internal stock is a min/max question, not a supplier one. No outbound CTA. */
  ...
  if (s.paymentTermsDays === null) {
    return {
      ...
      icon: "send",
      subtitle: "Net 60 letter signed by treasury",
    };
  }
  ...
  chase: (s) => ({
    steps: [
      ...
      },
      {
        ...
        text: `A one-page mail to ${s.name} asking for the ETA and the reason the line moved — and which open work orders it lands on. Signed by indirect sourcing, ready to go under your name.`,
        source: `Mercer · chase draft`,
      },
    ],
    outcomeTitle: "Chase drafted — ready to send",
    outcomeLine: `${s.name}: the mail is queued in the outbox and their blanket-release refs are attached. Sending it opens the ETA question with them.`,
  }),
  terms: (s) => ({
    steps: [
      {
        label: "Read what is on file",
        text: `${s.name} has ${SUPPLIER_STATUS_LABEL[s.status].toLowerCase()} standing and no payment term recorded on any of the three vendor masters. The Net 60 play is blocked until the letter is countersigned.`,
        source: `Supplier record · ${s.id}`,
      },
      {
        label: "Drafted the terms letter",
        text: `Net 60 letter drafted against the indirect sourcing policy, signed by treasury, addressed to the commercial contact of record.`,
        source: `Mercer · terms letter`,
      },
    ],
    outcomeTitle: "Terms letter — ready to send",
    outcomeLine: `${s.name}: letter is in the outbox with treasury's signature. Sending it opens Net 60 negotiation formally.`,
  }),
  consolidate: (s) => ({
    steps: [
      {
        label: "Read the category share",
        text: `${s.name} holds ${s.categoryShare}% of ${s.categories[0]}. The book carries this vendor plus an incumbent above the 50% line — the case for the incumbent carrying the overlap is arithmetic.`,
        source: `Category book · ${s.categories[0]}`,
      },
      {
        label: "Ran the benchmark",
        text: `Landed-cost model across the vendors on the family with a ranked shortlist attached. ${s.name} sits second on the scorecard.`,
  ...
  shortlist: (s) => ({
    steps: [
      ...
      },
      {
        ...
        text: `RFQ package with volumes, the fluid spec register, and the delivery window Indy Central Stores asks for. Ready to send to the shortlisted set.`,
    ...
    steps: [
      {
        ...
        text: `${s.name} is on exit — the case has been made, the successor is on the shortlist, and the window to wind the volume down needs a date on the PM calendar.`,
        source: `Supplier record · ${s.id}`,
      },
      {
        label: "Drafted the exit timeline",
        text: `Three-quarter timeline against current blanket cover, with the last committed release and the handover date to the successor vendor.`,
        source: `Mercer · exit timeline`,
      },
    ],
    outcomeTitle: "Exit timeline drafted",
    outcomeLine: `${s.name}: timeline is on the supplier page. Confirming it publishes the wind-down to indirect procurement and treasury.`,
  }),
  score: (s) => ({
    steps: [
      {
