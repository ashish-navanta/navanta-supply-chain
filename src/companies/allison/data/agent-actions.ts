  ...
  return {
    ...
    steps: [
      ...
      },
      {
        ...
        text: `Passed over with ${row.ref} attached: one account, ${formatUsd(row.value)}, a crew re-booked at our cost, and a PM window that goes back to ${install} if the factory can find an earlier slot.`,
    ...
    outcome: {
      ...
      lines: [
        ...
        `If an earlier slot comes back, the PM window can be pulled forward and I will tell her before it is booked.`,
      ],
    },
  };
}
...
export function agentTaskFor(
  ...
  switch (row.signal) {
      ...
      return {
        ...
        steps: [
          ...
          },
          {
            ...
            text: `Raising the level does not fill a shelf today. Kirby Risk Supply Co is ${bridge} ${row.qtyUnit} over target on a matching batch and can release the same day, which holds this node while the new level builds.`,
        ...
        outcome: {
          ...
          lines: [
            `Sized on the ${cover?.leadDays ?? 0}-day lead time we accepted, and Kirby Risk Supply Co covers ${row.party} while the level builds.`,
          ],
      ...
      return {
        ...
        record: {
            ...
            turns: [
              ...
              {
                speaker: first,
                text: `How bad? We have a shutdown crew booked for ${install} and that date is not easy to move.`,
              },
              {
                speaker: "Christy",
              ...
              {
                speaker: first,
                text: `The client specified this style for the lobby — they will not take a substitute. We will wait for the original.`,
              },
              {
                speaker: "Christy",
        ...
        steps: [
          {
            ...
            text: `The crew is booked for ${install} against a ${row.date} arrival. Moving the promise to ${newDate} pushes the PM window to ${newInstall}, and the re-book is at our cost because the slip is ours. ${formatUsd(row.value)} on ${row.qtyValue} ${row.qtyUnit}, contract price unchanged.`,
            source: "SAP ECC · install calendar · pricing held",
          },
          {
            label: `Moved the promise to ${newDate}`,
            text: `Written against ${row.ref}, so tracking, the account portal and the PM calendar all read the same date rather than three.`,
        ...
        outcome: {
          ...
          suggestion: up
            ? {
                ...
                body: `${first} has taken the later date, but the plant's slip is a buying decision and it is still open on ${up.party}. If Mercer can pull an earlier slot the PM window moves back toward ${install}. I have not asked — that is the buying desk's call, not mine.`,
              }
            : undefined,
        },
        next: up
    ...
    /* The plant took the substitute. Nothing needs re-promising — the PM window
    ...
    case "alternate-accepted": {
      ...
      /* "Deep Groove Ball Bearing 6205-2RS · Maple Frost" → "Maple Frost". The swap is
         a different make of the same part, which is what the order's own `said` field
         describes — so the call talks about colour rather than construction.
         Construction is form, fit and function, and on the same part in a
         different colour it cannot have changed: it had Greg asking after the
         one thing that is guaranteed identical. */
      const variant = proposal?.style.split(" · ").slice(-1)[0] ?? "the alternate";
      return {
        ...base,
        ...
        steps: [
          {
            ...
            text: `Same form and fit, same contract price on ${row.qtyValue} ${row.qtyUnit}, and enough on hand to ship the whole order rather than split it. The PM window on ${install} does not move.`,
          ...
          },
          {
            ...
            text: `Offered ${variant} at ${alternateEta} against waiting for the original on ${proposal?.date ?? install}. They chose to wait — the shutdown crew is not in until then, and they would rather not brief them on a different make.`,
            source: who.phone,
            call: {
              with: `${who.name} · ${row.party}`,
              duration: "0:45",
              /* NO AUDIO, deliberately. The recording on disk is the Shaw
                 ...
                 transcript and drops the player until an Allison take is
              ...
              turns: [
                ...
                { speaker: first, text: `Yes.` },
                {
                  ...
                  text: `Hi ${first}, this is Christy from Allison. I'm calling about order ${row.ref}. There are two options. The original order keeps the manufacturer you specified and delivers ${spokenDate(proposal?.date ?? install)}. Or we have the same part in ${variant} in stock at ${proposal?.at ?? "the stores"}, which would deliver ${spokenDate(alternateEta)}. Which would you prefer?`,
                },
                { speaker: first, text: `Same part, just a different make?` },
                {
                  speaker: "Christy",
                  text: `Same part, same form and fit, same contract price. The only difference is the manufacturer on the box.`,
                },
        ...
        outcome: {
          ...
          lines: [
            proposal
              ? `${first} was offered ${variant} at ${alternateEta} and chose to wait.`
        ...
        steps: [
          {
            ...
            text: `Options went out and nothing has come back, with the PM window closing.`,
            source: "Email thread · this order",
          },
          {
    ...
    case "pickup-window": {
      ...
      const crew = load?.installOn ?? "the PM window";
       ...
       * row uses the templated transcript until an Allison take is recorded;
      ...
      const recordedTurns: CallTurn[] = [
        ...
        {
          speaker: "Tova",
          text: `Hi Greg, this is Tova from Allison. I'm calling about order SO-4390, arriving September 6th between 8:00 and 11:00 AM. Does that window work for you?`,
        },
      ...
      const templatedTurns: CallTurn[] = [
        {
          speaker: "Tova",
          text: `${siteFirst}, it is about ${row.ref} into you on ${eta}. Two things — somebody on the dock to take it, and whether the ${crew} PM window is still standing.`,
        },
        {
          speaker: siteFirst,
