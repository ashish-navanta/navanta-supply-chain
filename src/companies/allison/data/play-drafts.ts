 ...
 * with this?
 *
 * The plays themselves are the 19 commodity plays cut from Allison's
 * Industrial Supplies cube — $34.1M over 1,093 vendors, $11.9M addressable —
 * and the routing rule behind the Lever column is the deck's: a winner share
 * of 50% or more consolidates to the incumbent, anything less goes to a
 * competitive RFP. OEM sole-source spend is carved out before either.
 */
...
/**
 * Short action-verb label per PlayKind — the "Lever" column vocabulary from the
 * Category & Opportunity Management view of Allison's MRO cube.
 *
...
export const LEVER_ACTION: Record<PlayKind, { label: string; blurb: string }> = {
  "index-hedge": { label: "Review index", blurb: "review the feedstock price hedge" },
...
const READY_BY_KIND: Record<PlayKind, { label: string; icon: PlayDraftIcon; subtitle: string }> = {
  consolidation: { label: "Approve shortlist", icon: "commit", subtitle: "Ranked distributor RFQ ready" },
  "dual-source": { label: "Approve benchmark", icon: "commit", subtitle: "Landed-cost model built, expedite premium included" },
  tariff: { label: "Approve reclass", icon: "commit", subtitle: "HTS reclass memo drafted for imported spares" },
  ...
  "pack-moq": { label: "Approve MOQ change", icon: "email", subtitle: "Distributor pack-size note drafted" },
  tail: { label: "Approve rollup", icon: "commit", subtitle: "Tail vendors clustered to the incumbent" },
  "index-hedge": { label: "Approve hedge", icon: "commit", subtitle: "Steel / resin index clause drafted" },
  "make-vs-buy": { label: "Approve make case", icon: "commit", subtitle: "Tool-room rebuild vs OEM quote costed" },
  rfp: { label: "Approve RFP", icon: "send", subtitle: "RFP scope ready · 19-play cube" },
};
...
export function playTaskFor(p: Play, agent: string): AgentTask | null {
  ...
  const evidence1Source = p.evidence[0]?.source ?? "Spend cube";
  ...
  if (d.intent === "approve") {
    ...
    const evidence2Source = p.evidence[1]?.source ?? "Vendor master";
    const steps = [
      {
        label: "Read the case",
        text:
  ...
  if (d.intent === "log" || d.intent === "recover") {
    ...
    const steps = [
      ...
      },
      {
        ...
        text: drifted
          ? "Recovery move drafted against the drifted milestone — a rebalance of the release to the second distributor with the quote already attached."
  ...
  if (d.intent === "archive") {
    const steps = [
      ...
      },
      {
        ...
        text: "A short internal note is ready with the number, the vendor consolidated, and the buyer who ran it.",
    ...
    return {
      ...
      outcome: {
        ...
        lines: [`${p.id} · The note is ready on the value page. One click sends it to the indirect procurement channel.`],
        continueLink,
      },
    };
  }
